...
def parse_kafka_configuration_page_v1(soup: BeautifulSoup) -> dict:
    ...
    for i, tr in enumerate(table.find_all("tr")):
        ...
        config[KafkaBrokerConfig.name][config_name]["default"] = extract_elt(
            "td", tr, 1
        )
        config[KafkaBrokerConfig.name]["description"] = extract_elt("td", tr, 2)
        config[KafkaBrokerConfig.name]["scope"] = (
