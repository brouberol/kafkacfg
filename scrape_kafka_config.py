...
KAFKA_VERSIONS = (
    '0.7', '0.8', '0.8.1', '0.8.2', '0.9.0', '0.10.0', '0.10.1', '0.10.2', '0.11.0',
    '1.0', '1.1', '2.0', '2.1', '2.2', '2.3', '2.4', '2.5', '2.6', '2.7', '2.8', '3.0',
    '3.1', '3.2', '3.3', '3.4'
)
...
def kafka_configuration_page_html(version: KafkaVersion) -> str:
    documentation_url = KAFKA_DOCUMENTATION_URL.format(version=version.to_url_fragment())
...
def parse_kafka_configuration_page_v4(soup: BeautifulSoup) -> dict:
    ...
    for li in ul.find_all('li', recursive=False):
        ...
        description_elts = list(takewhile(lambda elt: elt.name != 'ul', li.contents[1:]))
        ...
        config[config_name]['default'] = extract_elt('li', config_table, 1, post_processor)
        config[config_name]['valid_values'] = extract_elt('li', config_table, 2,
                                                          post_processor)
        config[config_name]['importance'] = extract_elt('li', config_table, 3,
                                                        post_processor)
        config[config_name]['update_mode'] = extract_elt('li', config_table, 4,
                                                         post_processor)
    return config
